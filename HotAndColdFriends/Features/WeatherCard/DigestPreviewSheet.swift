...
// MARK: - Digest Preview Sheet

/// A bottom-sheet that previews a daily digest weather card
/// and offers sharing via the system share sheet or Instagram Stories.
struct DigestPreviewSheet: View {
    ...
    private var cardPreview: some View {
        DailyDigestCardView(friends: friends)
            .scaleEffect(0.55)
            .frame(
                width: 390 * 0.55,
                height: 693 * 0.55
    ...
    private func prepareShareContent() async {
        renderedImage = WeatherCardRenderer.renderDigest(friends: friends)
        ...
        if let token = try? await inviteService.getOrCreateInviteToken(for: uid, userService: userService) {
            ...
            shareText = "Today's weather for my \(friends.count) friends ☀️ \(url.absoluteString)"
        } else {
            shareText = "Today's weather for my \(friends.count) friends ☀️"
        }
    }
}
